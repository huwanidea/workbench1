...
const workbenchMarkup = `
  <div class="app-shell">
    <header class="topbar">
      ...
      <div class="topbar-right"><div id="user-switcher"></div><div class="top-actions"><button class="button ghost" data-action="import-trigger">导入 Excel</button><button class="button ghost" data-action="export-json">备份 JSON</button><button class="button primary" data-action="export-excel">导出 Excel</button><input id="excel-input" type="file" accept=".xlsx,.xls" hidden></div></div>
    </header>
    <div class="shell-body">
      <aside class="sidebar">
        <div>
          <div class="sidebar-head"><span>工作区</span><button class="sidebar-toggle" data-action="toggle-sidebar" aria-label="收起侧边栏">‹</button></div>
          <nav class="nav-list">
            ...
            <button class="nav-item" data-tab="settings"><span class="nav-short">规</span><span class="nav-label">规则与参数</span></button>
          </nav>
        </div>
        <div class="side-note"><span class="status-dot"></span><span>本地数据已自动保存</span></div>
      </aside>
